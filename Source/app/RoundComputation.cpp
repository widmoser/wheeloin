...
const int EXPLORATION_RANGE = 10;
...
void RoundComputation::computeFragmentedScore(Score& score, Parameters& parameters, int series[12]) {
    double time = 10.0;
    Note firstBass(3, rand() % 12 + 26, time, 0.0, 1.0, 1.0);
    ...
    for (int i = 0; i < 100; ++i) {
        ...
        if (rand() % 10 < 7) {
            ...
            Note n(3, rand() % 12 + 26, time, 0.0, 1.0, 1.0);
            score.addNote(n);
    ...
    const std::vector<Note> v = score.getNotes();
    for (std::vector<Note>::const_iterator i = v.begin(); i != v.end(); ++i) {
        std::cout << "(" << i->voice << ", " << i->start << ")" << std::endl;
    }
}
